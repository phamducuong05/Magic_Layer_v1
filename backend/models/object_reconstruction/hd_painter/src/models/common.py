...
from ..smplfusion import DDIM, share, scheduler
from ..utils.convert_diffusers_to_sd import (
    ...
    try:
        ...
        if not AUTO_DOWNLOAD_ALLOWED:
            raise FileNotFoundError(
                f'Checkpoint is missing and automatic download is disabled: {save_path}'
            )
...
def get_obj_from_str(string):
    ...
    package_root = __package__.rsplit(".models", 1)[0]
    if not module.startswith(package_root):
        module = f"{package_root}.{module}"
...
def load_sd_inpainting_model(
    ...
    vae = load_obj(f'{CONFIG_FOLDER}/vae.yaml').eval().to(device)

    # Load version specific config files
    if sd_version == 1:
        encoder = load_obj(f'{CONFIG_FOLDER}/encoders/clip.yaml').eval().to(device)
        unet = load_obj(f'{CONFIG_FOLDER}/unet/inpainting/v1.yaml').eval().to(device)
    elif sd_version == 2:
        encoder = load_obj(f'{CONFIG_FOLDER}/encoders/openclip.yaml').eval().to(device)
        unet = load_obj(f'{CONFIG_FOLDER}/unet/inpainting/v2.yaml').eval().to(device)
    else:
