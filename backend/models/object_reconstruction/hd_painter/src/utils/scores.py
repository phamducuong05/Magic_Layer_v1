...
def l1(_crossattn_similarity, mask, token_idx = [1,2]):
    ...
    return (similarity[token_idx] * mask.to(similarity.device)).sum()
...
def bce(_crossattn_similarity, mask, token_idx = [1,2]):
    ...
    return -sum([
        F.binary_cross_entropy_with_logits(x - 1.0, mask.to(x.device))
...
def softmax(_crossattn_similarity, mask, token_idx = [1,2]):
    ...
    return (score * mask.to(score.device)).sum() # Sum up log-probabilities (equivalent to multiplying P-values) for all pixels inside of the mask
