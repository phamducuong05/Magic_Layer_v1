...
class DDIM:
    ...
    def __call__(
        ...
        device = self.vae.encoder.conv_in.weight.device
        if unet_condition is not None:
            zT = torch.randn(
                (1,4) + unet_condition.shape[2:], device=device
            )
        else:
            ...
            for timestep in share.DDIMIterator(pbar):
                ...
                eps_uncond, eps = self.unet(
                    ...
                    timesteps = torch.tensor(
                        [timestep, timestep], device=device
                    ), 
                    context = context
                ).chunk(2)
    ...
    def get_inpainting_condition(self, image, mask):
        ...
        device = self.vae.encoder.conv_in.weight.device
        with torch.no_grad():
            masked_image = image.torch().to(device) * ~mask.torch(0).bool().to(device)
            masked_image = masked_image.to(dtype)
            condition_x0 = self.vae.encode(masked_image).mean * self.config.scale_factor
        condition_mask = (
            mask.resize(latent_size[::-1]).to(device).torch(0).bool().to(dtype)
        )
